...
impl Budget {
    ...
    /// as `probe` does, read as [`Budget::configured`] reads the environment.
    ///
    /// Separate so the rule can be tested against a machine a test states,
    /// without changing the process environment, which Rust 2024 makes
    /// `unsafe`.
    ///
    /// # Errors
    ///
    /// Returns a [`Failure`] when the value is not a number.
    pub fn from_variable(value: Option<OsString>, probe: Probe) -> Result<Self, Failure> {
