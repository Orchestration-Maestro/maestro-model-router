...
fn a_router_authored_reply_is_readable_by_a_browser_client() {
    ...
    assert_eq!(
        ...
        Some("*"),
        "what the router authors is open to any page that can reach it, \
         because this header is not what decides that reach; without it a \
         preflight passes and the answer it cleared is then refused:\n{reply}"
    );
}
