//! What the catalog charges an entry, when it declares no estimate or one
//! the files contradict.
//!
//! Each case writes model files in the format's own layout under a scratch
//! root and reads a one-entry catalog against them, so every figure asserted
//! here can be worked out by hand from the file the test just wrote.

use std::fs;

use maestro_model_router::catalog::{Catalog, EstimateSource};
...
/// What one model's entry is estimated at, beside 64 MiB of weights.
pub(super) fn estimated(label: &str, model: &Gguf) -> u32 {
...
///
/// Intentional coupling to `estimate::OVERHEAD_BYTES`, as `CARD_MIB` in
/// `entry_schema` is to this machine: an entry holding no layers on the device pays this and
