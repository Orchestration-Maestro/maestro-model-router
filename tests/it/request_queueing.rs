...
use crate::support::{MODEL, ModelsRoot, Serving, post, queued, request, settled, status};
...
fn contended_and_streaming() -> (Serving, TcpStream) {
    let serving = queued(
        &contended(),
        ModelsRoot::with(&[MODEL, SECOND_MODEL]),
        Some(4096),
...
fn in_line(serving: &Serving, count: usize) {
