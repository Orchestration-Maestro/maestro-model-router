...
//!
//! The case that needs the named build found runs the router as a process
...
fn an_entry_naming_a_runtime_is_served_from_that_build() {
    ...
    fs::write(&catalog, one_entry(Some("fastmtp"))).expect("a catalog in the temporary directory");
    let search = SearchPath::with_stub().carrying("fastmtp");

    let mut router = RouterProcess::serve(&catalog, &root, &search);
    let reply = request(router.address(), &get("/models/gemma3/v1/echo"));
    assert_eq!(
        status(&reply),
        Some(200),
