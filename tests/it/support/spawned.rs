...
//!
//! Moved here from `signalled_shutdown.rs` when a second test needed it. A test that
...
impl RouterProcess {
    ...
    pub(crate) fn exited_within(&mut self, deadline: Duration) -> Option<ExitStatus> {
        polled(deadline, Duration::from_millis(25), || {
            self.process.try_wait().expect("the router's status")
        })
    }
